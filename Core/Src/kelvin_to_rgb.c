...
void pulse_for_kelvin(uint32_t kelvin, KelvinToRGB *lower, KelvinToRGB *higher, uint16_t *pulse_values) {
	...
	if (kelvin != lower->kelvin) {
		rgb_values[0] = round(lower->r + (double)(kelvin - lower->kelvin) / (higher->kelvin - lower->kelvin) * (higher->r - lower->r));
		rgb_values[1] = round(lower->g + (double)(kelvin - lower->kelvin) / (higher->kelvin - lower->kelvin) * (higher->g - lower->g));
		rgb_values[2] = round(lower->b + (double)(kelvin - lower->kelvin) / (higher->kelvin - lower->kelvin) * (higher->b - lower->b));
	}
	else {
    ...
    printf("\nLower pulse values: %lu = (%lu, %lu, %lu)\n", lower->kelvin, lower->r, lower->g, lower->b);
    printf("Higher pulse values: %lu = (%lu, %lu, %lu)\n", higher->kelvin, higher->r, higher->g, higher->b);
    printf("Interpolated RGB: %lu = (%lu, %lu, %lu)\n", kelvin, rgb_values[0], rgb_values[1], rgb_values[2]);
